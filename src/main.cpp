...
struct __attribute__((__packed__)) RootArg {
    ...
    /*6*/ CAmbisonicBinauralizer* decoder;
    /*7*/ size_t bytes_decoder;
    /*8*/ float** resultSample;
...
void setAndFFT_fxp(/*0*/ CAmbisonicBinauralizer* decoder, /*1*/ size_t bytes_decoder, /*2*/ CBFormat* sumBF, /*3*/ size_t bytes_sumBF) {
    __hpvm__hint(hpvm::CPU_TARGET);
...
// Overlap
void overlap_left_fxp(/*0*/ CAmbisonicBinauralizer* decoder, /*1*/ size_t bytes_decoder, /*2*/ float** resultSample, /*3*/ size_t bytes_resultSample) {
    __hpvm__hint(hpvm::CPU_TARGET);
    ...
    memcpy(resultSample[0], decoder->m_pfScratchBufferA.data(), (decoder->m_nBlockSize) * sizeof(float));
...
void overlap_right_fxp(/*0*/ CAmbisonicBinauralizer* decoder, /*1*/ size_t bytes_decoder, /*2*/ float** resultSample, /*3*/ size_t bytes_resultSample) {
    __hpvm__hint(hpvm::CPU_TARGET);
    __hpvm__attributes(2, decoder, resultSample, 1, resultSample);
    ...
    memcpy(resultSample[1], decoder->m_pfScratchBufferA.data(), (decoder->m_nBlockSize) * sizeof(float));
    for(int ni = 0; ni < decoder->m_nOverlapLength; ni++) {
...
void audioDecoding(/*0*/ CAmbisonicProcessor* rotator, /*1*/ size_t bytes_rotator, /*2*/ CBFormat* sumBF, /*3*/ size_t bytes_sumBF, /*4*/ CAmbisonicZoomer* zoomer, \
    ...
    void* setAndFFTNode = __hpvm__createNodeND(0, setAndFFT_fxp);
    ...
    __hpvm__bindIn(setAndFFTNode, 6, 0, 0);    // Ying: TODO: bindIn and edge the previous nodes, and check the pipeline example
    __hpvm__bindIn(setAndFFTNode, 7, 1, 0);
    __hpvm__bindIn(setAndFFTNode, 2, 2, 0);
    __hpvm__edge(zoomProcessNode, setAndFFTNode, 1, 0, 3, 0);
    
    __hpvm__bindIn(FIR_LeftNode, 6, 0, 0);
    __hpvm__edge(setAndFFTNode, FIR_LeftNode, 1, 0, 1, 0);
    
    __hpvm__bindIn(FIR_RightNode, 6, 0, 0);
    __hpvm__edge(setAndFFTNode, FIR_RightNode, 1, 1, 1, 0);

    __hpvm__bindIn(IFFT_LeftNode, 6, 0, 0);
    __hpvm__edge(FIR_LeftNode, IFFT_LeftNode, 1, 0, 1, 0);

    __hpvm__bindIn(IFFT_RightNode, 6, 0, 0);
    ...
    __hpvm__bindIn(overlap_RightNode, 6, 0, 0);
    __hpvm__edge(IFFT_RightNode, overlap_RightNode, 1, 0, 1, 0);
    __hpvm__bindIn(overlap_RightNode, 8, 2, 0);
    __hpvm__bindIn(overlap_RightNode, 9, 3, 0);
